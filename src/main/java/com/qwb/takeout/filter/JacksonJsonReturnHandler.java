...
import com.qwb.takeout.annotation.JsonFilters;
...
@Slf4j
public class JacksonJsonReturnHandler implements HandlerMethodReturnValueHandler, BeanPostProcessor {
    List<ResponseBodyAdvice<Object>> advices = new ArrayList<>();

    @Override
    public boolean supportsReturnType(MethodParameter returnType) {
        boolean hasJSONAnno = returnType.getMethodAnnotation(JsonFilter.class) != null || returnType.getMethodAnnotation(JsonFilters.class) != null;
        return hasJSONAnno;
    }
    ...
    public void handleReturnValue(Object returnValue, MethodParameter returnType, ModelAndViewContainer mavContainer,
        ...
        CustomJacksonJsonSerializer jsonSerializer = new CustomJacksonJsonSerializer();
        Arrays.asList(annos).forEach(a -> {
            if (a instanceof JsonFilter) {
                JsonFilter json = (JsonFilter) a;
                jsonSerializer.filter(json);
            } else if (a instanceof JsonFilters) {
                JsonFilters jsons = (JsonFilters) a;
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        if (bean instanceof ResponseBodyAdvice) {
            ...
            JacksonJsonReturnHandler jsonHandler = null;
            for (int i = 0; i < handlers.size(); i++) {
                HandlerMethodReturnValueHandler handler = handlers.get(i);
                if (handler instanceof JacksonJsonReturnHandler) {
                    jsonHandler = (JacksonJsonReturnHandler) handler;
                    break;
                }
            }
            if (jsonHandler != null) {
